...
# -test lifo_1 ==> has extrac indirections

make opt && prun output.opt -test real_lifo -seq ocaml_list,vector,chunked_stack,circular_array,sized_array -n 20000000 -length 10,100,1000,10000,100000,1000000,10000000 -runs 3

cp plots.pdf plots/plots_lifo.pdf
cp results.txt plots/results_lifo.txt
...
make opt && prun output.opt -test lifo_1 -seq chunked_stack -n 50000000 -length 10,1000,100000,10000000,50000000 -chunk 32,64,128,256,512,1024 -runs 3

##==> todo: investigate issue with outofbound.

cp plots.pdf plots/plots_chunk_size.pdf
cp results.txt plots/results_chunk_size.txt
...
make opt && prun output.opt -test real_lifo,lifo_1 -seq ocaml_list,vector,chunked_stack,circular_array,sized_array -n 20000000 -length 10,100,1000,10000,100000,1000000,10000000 --append
# -runs 3
