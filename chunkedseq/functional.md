...
tabulate γ (n, f)           : α wf → int ✕ (int → α) → α chunk = (Σ γ xs, xs)
  where xs = [f 0, ..., f (n - 1)]
~~~~~

~~~~~ {.ocaml}
ec : α wf → chunk
ec γ = tabulate γ (0, λ x. 1)
~~~~~
...
check γ (d as Deep (_, {fₒ, fᵢ, m, bᵢ, bₒ})) =
  ...
  if sz == 0 && ¬ (empty m) then
    mk_deep γ {fₒ=pop_front m, fᵢ=fᵢ, m=m, bᵢ=bᵢ, bₒ=bₒ}
  else if sz ≤ 1 && empty m then
    mk_shallow' γ (fₒ, fᵢ, bᵢ, bₒ)
  else
    d
~~~~~
...
push_front' γ (Shallow c) x =
  if Chunk.full c then
    let m = Shallow (ec γ) in
    push_front' γ (mk_deep γ {fₒ=ec γ, fᵢ=ec γ, m=m, bᵢ=ec γ, bₒ=c}, x)
  else
    Shallow (Chunk.push_front γ (c, x))
push_front' γ (Deep (_, {fₒ, fᵢ, m, bᵢ, bₒ})) =
  if Chunk.full fₒ then
    if Chunk.empty fᵢ then
      push_front' γ (mk_deep γ {fₒ=fᵢ, fᵢ=fₒ, m=m, bᵢ=bᵢ, bₒ=bₒ}, x)
    else
      let m' = push_front' (Σ weight) (m, fᵢ) in
      push_front' γ (mk_deep γ {fₒ=ec γ, fᵢ=fₒ, m=m', bᵢ=bᵢ, bₒ=bₒ}, x)
  else
...
pop_front' γ (Deep (_, {fₒ, fᵢ, m, bᵢ, bₒ})) =
  if Chunk.empty fₒ then
    if ¬ (Chunk.empty fᵢ) then
      pop_front' γ (mk_deep' γ {fₒ=fᵢ, fᵢ=fₒ, m=m, bᵢ=bᵢ, bₒ=bₒ})
      ...
      pop_front' γ (mk_deep' γ {fₒ=bₒ, fᵢ=fᵢ, m=m, bᵢ=bᵢ, bₒ=fₒ})
    else
...
push_buffer_back' γ (Shallow c', c)  =
  mk_deep' γ {fₒ=c', fᵢ=ec γ, m=Shallow (ec γ), bᵢ=ec γ, bₒ=c}
push_buffer_back' γ (Deep (_, {fₒ, fᵢ, m, bᵢ, bₒ}), c)  =
  if Chunk.weight c + Chunk.weight bₒ ≤ K then
    let bₒ' = Chunk.concat γ (bₒ, c) in
    mk_deep' γ {fₒ=fₒ, fᵢ=fᵢ, m=m, bᵢ=bᵢ, bₒ=bₒ'}
  else
    let m' =
...
split' γ (Deep (_, {fₒ, fᵢ, m, bᵢ, bₒ})) =
  ...
  let (s₁, x, s₂) = 
    if i ≤ wfₒ then
      ...
      let s₁ = mk_deep γ {fₒ=fₒ₁, fᵢ=ec γ, m=Shallow (ec γ),
                          bᵢ=ec γ, bₒ=ec γ}
      in
      let s₂ = mk_deep γ {fₒ=fₒ₂, fᵢ=fᵢ, m=m, bᵢ=bᵢ, bₒ=bₒ} in
      (s₁, x, s₂)
    else if i ≤ wfₒ + wfᵢ then
      let (fᵢ₁, x, fᵢ₂) = Chunk.split γ (fᵢ, i) in
      let s₁ = mk_deep γ {fₒ=fₒ, fᵢ=ec γ, m=Shallow (ec γ),
                          bᵢ=ec γ, bₒ=fᵢ₁} in
      let s₂ = mk_deep γ {fₒ=fᵢ₂, fᵢ=ec γ, m=m, bᵢ=bᵢ, bₒ=bₒ} in
      (s₁, x, s₂)
    else if i ≤ wfₒ + wfᵢ + wₘ then
      let j = i - wfₒ - wfᵢ in
      let (m₁, c, m₂) = split' (Σ weight) (m, j) in
      let (c₁, x, c₂) = Chunk.split γ (c, j - weight m₁) in
      let s₁ = mk_deep γ {fₒ=fₒ, fᵢ=fᵢ, m=m₁, bᵢ=ec γ, bₒ=c₁} in
      let s₂ = mk_deep γ {fₒ=c₂, fᵢ=ec γ, m=m₂, bᵢ=bᵢ, bₒ=bₒ} in
      (s₁, x, s₂)
